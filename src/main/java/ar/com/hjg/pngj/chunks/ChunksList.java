...
public class ChunksList {
  ...
  /**
   * WARNING: this does NOT return a copy, but the list itself. The called should not modify this
   * directly! Don't use this to manipulate the chunks.
   */
   ...
   * <p>
   * If more than one chunk is found, then an exception is thrown (failifMultiple=true or chunk is
   * single) or the last one is returned (failifMultiple=false)
   **/
   ...
   * Returns only one chunk or null if nothing found - does not include queued
   * <p>
   * If more than one chunk (after filtering by inner id) is found, then an exception is thrown
   * (failifMultiple=true or chunk is single) or the last one is returned (failifMultiple=false)
   **/
