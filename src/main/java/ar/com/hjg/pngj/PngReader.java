...
public class PngReader implements Closeable {
    ...
    /**
     * This object has most of the intelligence to parse the chunks and decompress
     * the IDAT stream
     */
     ...
     * cursor, storing only one (the current) row. This stores several (perhaps all)
     * rows only if calling {@link #readRows()} or for interlaced images (this later
     * is transparent to the user)
     */
    protected IImageLineSet<? extends IImageLine> imlinesSet;

    /**
     * This factory decides the concrete type of the ImageLine that will be used.
     * See {@link ImageLineSetDefault} for examples
     */
    ...
    /**
     * Constructs a PngReader object from a stream, with default options. This reads
     * the signature and the first IHDR chunk only.
     * <p>
    ...
    /**
     * Same as {@link #PngReader(InputStream)} but allows to specify early if the
     * stream must be closed
     * 
     * @param inputStream
     * @param shouldCloseStream The stream will be closed in case of exception
     *                          (constructor included) or normal termination.
     */
    ...
    /**
     * Constructs a PngReader opening a file. Sets <tt>shouldCloseStream=true</tt>,
     * so that the stream will be closed with this object.
     * 
     ...
     * first IDAT chunk id
     * <P>
     * This can be called several times (tentatively), it does nothing if already
     * run
     * <p>
     ...
     * settings before reading the first row? but sometimes we want to access some
     * metadata (plte, phys) before. Because of this, this method can be called
     * explicitly but is also called implicititly in some methods (getMetatada(),
     * getChunksList())
     */
    ...
    /**
     * All loaded chunks (metada). If we have not yet end reading the image, this
     ...
     * replaced by a single dummy-marker IDAT chunk. These might be copied to the
     * PngWriter
     * <p>
     * 
     * @see #getMetadata()
    ...
    /**
     * @param forceLoadingOfFirstChunks If true, chunks before IDAT will be load if
     *                                  needed
    ...
    /**
     * Sets the factory that creates the ImageLine. By default, this implementation
     * uses ImageLineInt but this can be changed (at construction time or later) by
     * calling this method.
     * <p>
    ...
    /**
     * Reads all the (remaining) file, skipping the pixels data. This is much more
     * efficient that calling {@link #readRow()}, specially for big files (about 10
     * times faster!), because it doesn't even decompress the IDAT stream and
     * disables CRC check Use this if you are not interested in reading pixels,only
     * metadata.
     */
     ...
     * These are the bytes read (not loaded) in the input stream. If exceeded, an
    ...
    /**
     * Set total maximum bytes to load from ancillary chunks (0: unlimited; default:
     * 5Mb).<br>
     ...
     * default: 2MB). <br>
     * Chunks exceeding this length will be skipped (the CRC will not be checked)
     * and the chunk will be saved as a PngChunkSkipped object. See also
     * setSkipChunkIds
     */
     ...
     * These chunks will be skipped (the CRC will not be checked) and the chunk will
     * be saved as a PngChunkSkipped object. See also setSkipChunkMaxSize
     */
     ...
     * trailing chunks and close the stream. But it can be called at anytime. This
     * will also read the first chunks if not still read, and skip pixels (IDAT) if
     * still pending.
     * 
    ...
    /**
     * Releases resources, and closes stream if corresponds. Idempotent, secure, no
     * exceptions.
     * 
     * This can be also called for abort. It is recommended to call this in case of
     * exceptions
     */
    public void close() {
	try {
    ...
    /**
     * Disables the CRC integrity check in IDAT chunks and ancillary chunks, this
     * gives a slight increase in reading speed for big files
     */
    ...
    /**
     * Pseudo 64-bits digest computed over the basic image properties and the raw
     * pixels data: it should coincide for equivalent images encoded with different
     * filters and compressors; but will not coincide for interlaced/non-interlaced;
     * also, this does not take into account the palette info. This will be valid
     * only if {@link #prepareSimpleDigestComputation()} has been called, and all
     * rows have been read. Not fool-proof, not cryptografically secure, only for
