...
public class PngWriter implements Closeable {
     ...
     * advanced when just starting to write the new group, not when finalizing the
     * previous)
     * <p>
     * see {@link ChunksList}
     */
     ...
     * @param file
     * @param imgInfo
     * @param allowoverwrite If false and file exists, an
     *                       {@link PngjOutputException} is thrown
     */
    ...
    /**
     * Constructs a new PngWriter from a output stream. After construction nothing
     * is writen yet. You still can set some parameters (compression, filters) and
     ...
     * {@link ChunkHelper#equivalent(PngChunk, PngChunk)), this overwrites it. <p>
    ...
    /**
     * Sets an origin (typically from a {@link PngReader}) of Chunks to be copied.
     * This should be called only once, before starting writing the rows. It doesn't
     * matter the current state of the PngReader reading, this is a live object and
     * what matters is that when the writer writes the pixels (IDAT) the reader has
     * already read them, and that when the writer ends, the reader is already ended
     * (all this is very natural).
     * <p>
     * Apart from the copyMask, there is some addional heuristics:
     * <p>
     * - The chunks will be queued, but will be written as late as possible (unless
     * you explicitly set priority=true)
     * <p>
     * - The chunk will not be queued if an "equivalent" chunk was already queued
     * explicitly. And it will be overwriten another is queued explicitly.
     * 
     * @param chunks
    ...
    /**
     * Copy all chunks from origin. See {@link #copyChunksFrom(ChunksList, int)} for
     * more info
     */
     ...
     * <p>
     * Actually: compressed size = total size of IDAT data , raw size = uncompressed
     * pixel bytes = rows * (bytesPerRow + 1).
     * 
     ...
     * <p>
     * This is normally called internally from {@link #end()}, you should only call
     * this for aborting the writing and release resources (close the stream).
     * <p>
    ...
    /**
     * This is kept for backwards compatibility, now the PixelsWriter object should
     * be used for setting compression/filtering options
     * 
     * @see PixelsWriter#setCompressionFactor(double)
     * @param compLevel between 0 (no compression, max speed) and 9 (max
     *                  compression)
     */
    ...
    /**
     * Sets maximum size of IDAT fragments. Incrementing this from the default has
     * very little effect on compression and increments memory usage. You should
     * rarely change this.
     * <p>
     * 
    ...
    /**
     * Factory method for pixels writer. This will be called once at the moment at
     * start writing a set of IDAT chunks (typically once in a normal PNG)
     * 
     * This should be overriden if custom filtering strategies are desired. Remember
     * to release this with close()
     * 
