...
public class DeflatedChunkSetTest extends PngjTest {
	...
	public static class ChunkSeqReaderIdatRawCb extends ChunkSeqReader { // callback
		...
		protected DeflatedChunksSet createIdatSet(String id) {
			DeflatedChunksSet cc = new DeflatedChunksSet(id, true, rowsize, rowsize) {
				@Override
	...
	public static class ChunkSeqReaderIdatRawP extends ChunkSeqReader {// normal (polled,sync)
		...
		@Override
		protected DeflatedChunksSet createIdatSet(String id) {
			DeflatedChunksSet cc = new DeflatedChunksSet(id, false, rowsize, rowsize) {
			};
			return cc;
		}
		...
		public void readFrom(InputStream is) {
			...
			if (!idatFound || getCurDeflatedSet() == null)
				throw new PngjInputException("premature ending before IDAT");
			for (rown = 0; rown < nrows; rown++) {
				while (getCurDeflatedSet().isWaitingForMoreInput()) {
					if (bf.feed(this) < 1)
						throw new PngjInputException("premature ending inflating rows");
				}
				if (getCurDeflatedSet().isRowReady()) {
					summary.append(TestSupport.showRow(getCurDeflatedSet().getInflatedRow(),
							getCurDeflatedSet().getRowFilled(), getCurDeflatedSet().getRown())).append(" ");
					getCurDeflatedSet().prepareForNextRow(rowsize);
				} else
					break;
			}
			if (getCurDeflatedSet() != null && !getCurDeflatedSet().isDone())
				getCurDeflatedSet().markAsDone();
			while (!isDone()) {
				if (bf.feed(this) < 1)
