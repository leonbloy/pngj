...
public class PngReaderTest extends PngjTest {
	...
	public void testReadBad() {
		IImageLineSet<ImageLineInt> linesok, lines;
		try (PngReader pngr2 = new PngReader(TestSupport.istream("test/bad_testrgb3_noend.png"))) {
			lines = (IImageLineSet<ImageLineInt>) pngr2.readRows();
			pngr2.end();
		}
		try (PngReader pngr1 = new PngReader(TestSupport.istream("test/testrgb3.png"))) {
			linesok = (IImageLineSet<ImageLineInt>) pngr1.readRows();
			pngr1.end();
		}
		TestCase.assertEquals(TestSupport.showLine(lines.getImageLine(0)),
				TestSupport.showLine(linesok.getImageLine(0)));
	}
