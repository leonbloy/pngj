...
public class Sandbox {
	...
	public static void convert(String origFilename, String destFilename) {
		...
		pngr.end(); // it's recommended to end the reader first, in case there are trailing chunks
		// to read
		pngw.end();
	}
	...
	public static void images3() throws IOException {
		...
		{
			BufferedImage bi3 = new BufferedImage(bi1.getWidth(), bi1.getHeight(), BufferedImage.TYPE_BYTE_INDEXED);
			bi3.getGraphics().drawImage(bi1, 0, 0, null);
			int p1i3 = bi3.getRGB(0, 0);
			int p2i3 = bi3.getRGB(1, 0);
			System.out.printf("im3: p1=%08x %s  p2=%08x %s\n", p1i3, formatARGB(p1i3), p2i3, formatARGB(p2i3));
		}
	}

	public static void imagesas() throws IOException {
		File png = new File("d:\\devel\\repositories\\pngj\\src\\test\\resources\\colormodels\\08ax2.png");
		BufferedImage bi1 = ImageIO.read(png);
		int p1i1 = bi1.getRGB(0, 0);
		int p2i1 = bi1.getRGB(1, 0);
	...
	public static String formatARGB2(int v) {
		int a = (v >> 24) & 0xFF;
		return String.format("(%d,%d,%d,%d) (%f,%f,%f)", (v >> 24) & 0xFF, (v >> 16) & 0xFF, (v >> 8) & 0xFF, v & 0xFF,
				a * ((v >> 16) & 0xFF) / 255.0, a * ((v >> 8) & 0xFF) / 255.0, a * (v & 0xFF) / 255.0);
	}
