...
public class SpyBi {
	...
	String res = "";
	res += String.valueOf(im.bitDepth) + (im.alpha ? "a" : "") + (im.indexed ? "p" : "")
		+ (im.greyscale ? "g" : "");
	...
	String boff = sampleModel instanceof PixelInterleavedSampleModel
		? Arrays.toString(((PixelInterleavedSampleModel) sampleModel).getBandOffsets())
		: "NA";
	...
	String res = String.format(type + ";cm=" + colorModelClass + ",ps=" + pixelSize + ",ha="
		+ (hasAlpha ? "Y" : "N") + ",ap=" + (isAlphaPre ? "Y" : "N") + ",cs=" + componentsSize + ";srgb="
		+ issRGB + ";sm=" + sampleModelClass + ";db=" + databufferclas + "," + bufferDataType + ",bo=" + boff);
	return res;
    }
