 ...
 * 
 * It's "opaque" in the sense that it does not parse the IHDR, and doesn't know
 * the PNG properties. It uncompress the IDAT, but returns it in arbitrary block
 * sizes (not row per row) . All this can be overriden
 */
public class ChunkSeqReaderOpaque extends ChunkSeqReader {
	...
	protected ChunkReader createChunkReaderForNewChunk(String id, int len, long offset, boolean skip) {
		return new ChunkReader(len, id, offset,
				skip ? ChunkReaderMode.SKIP : (bufferChunks ? ChunkReaderMode.BUFFER : ChunkReaderMode.PROCESS)) {
			@Override
	...
	protected DeflatedChunksSet createIdatSet(String id) {
		return new DeflatedChunksSet(id, true, blockSize, blockSize) {
			@Override
