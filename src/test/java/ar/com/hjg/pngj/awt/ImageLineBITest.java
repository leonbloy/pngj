...
public class ImageLineBITest {
  ...
  private void delOnExit(File f) {tmpFilesToDelete.add(f);}
  ...
  private void testRead2(File ori, boolean preferCustom) {
    PngHelperInternal.debug(String.format("====testing with values %s cust=%s==", ori.getName(), preferCustom));
    PngReaderBI png = new PngReaderBI(ori);
    png.setPreferCustomInsteadOfBGR(preferCustom);
    File dest = TestSupport.absFile("test/__test.tmp.png");
    delOnExit(dest);
  ...
  public void testWrite1(File f, int convToType, boolean forceRgb) {
    try {
      ...
      PngHelperInternal.debug(f + " type=" + ImageIoUtils.imageTypeName(bi1.getType()) + " conv to "
          + (convToType > -1 ? ImageIoUtils.imageTypeName(convToType) : "-") + " force RGB="
          + forceRgb);
      ...
      System.err.println("Error with " + f + " typeconv="
          + ImageIoUtils.imageTypeNameShort(convToType) + " cordeRgb=" + forceRgb);
      throw e;
    }
  }
  ...
  public void testWriteRgb8() {
    ...
    testWrite1(TestSupport.absFile("testsuite2/basn2c08.png"), BufferedImage.TYPE_4BYTE_ABGR_PRE,
        false); // this uses custom
  ...
  @After
  public void endTest(){
    if(removeTmpFiles) for (File f:tmpFilesToDelete) {
      f.delete();
    }
