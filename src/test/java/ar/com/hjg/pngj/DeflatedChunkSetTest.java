...
/**
 * Test the DeflatedChunkSet using a ChunkSetReader2 that reads the IDATs with no knowledge of
 * filters, etc
 */
public class DeflatedChunkSetTest extends PngjTest {
  ...
  public static class ChunkSetReaderIdatRaw extends ChunkSeqReader {// normal (polled,sync) callback
    ...
    public void readFrom(InputStream is) {
      ...
      for (rown = 0; rown < nrows; rown++) {
        while (!isDone() && getCurReaderDeflatedSet() != null
            && !getCurReaderDeflatedSet().isRowReady()) {
          if (bf.feed(this) < 1)
            break;
        }
  ...
  /*
   * public static void main(String[] args) throws Exception{ byte[] data= new
   * byte[]{0,42,43,0,44,41}; byte[] compressed = new byte[20]; Deflater def1 = new
   * Deflater(Deflater.DEFAULT_COMPRESSION); def1.setInput(data); def1.finish(); int n =
   * def1.deflate(compressed); System.out.println(n);
