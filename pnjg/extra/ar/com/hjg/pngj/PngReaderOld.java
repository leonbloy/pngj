...
public class PngReaderOld {
	 ...
	 * 
	 */
	public PngReaderOld(InputStream inputStream, String filenameOrDescription) {
	 ...
	 */
	public ImageLinesOld readRowsInt(int rowOffset, int nRows, int rowStep) {
		if (nRows < 0)
			nRows = (imgInfo.rows - rowOffset) / rowStep;
		if (rowStep < 1 || rowOffset < 0 || nRows * rowStep + rowOffset > imgInfo.rows)
			throw new PngjInputException("bad args");
		ImageLinesOld imlines = new ImageLinesOld(imgInfo, SampleType.INT, unpackedMode, rowOffset, nRows, rowStep);
		if (!interlaced) {
	 ...
	 */
	public ImageLinesOld readRowsInt() {
	 ...
	 */
	public ImageLinesOld readRowsByte(int rowOffset, int nRows, int rowStep) {
		if (nRows < 0)
			nRows = (imgInfo.rows - rowOffset) / rowStep;
		if (rowStep < 1 || rowOffset < 0 || nRows * rowStep + rowOffset > imgInfo.rows)
			throw new PngjInputException("bad args");
		ImageLinesOld imlines = new ImageLinesOld(imgInfo, SampleType.BYTE, unpackedMode, rowOffset, nRows, rowStep);
		if (!interlaced) {
			for (int j = 0; j < imgInfo.rows; j++) {
				int bytesread = readRowRaw(j); // read and perhaps discards
				int mrow = imlines.imageRowToMatrixRowStrict(j);
	 ...
	 */
	public ImageLinesOld readRowsByte() {
